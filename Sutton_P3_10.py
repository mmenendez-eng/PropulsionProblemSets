...
def real_max_exhaust_velocity(k,R,Pc,Tc,Pe,Te,eps):
    return Pe/(R*Te)*np.sqrt(k*R*Tc)/(eps*Pc*((k+1)/2)**(1/(k-1)))

## ANALYSIS
# Maximum Exhaust Velocity:
# - at Vacuum (ExpR -> Inf)
v_max = ideal_max_exhaust_velocity(k,R,T_c)

# - at given Expansion Ratio (ExpR = 2000)
Me = calculate_exit_mach_number(ExpR,k)
T_e = T_c/(1 + (k-1)/2*Me**2)
v_max_real = Me*np.sqrt(k*R*T_e)

## OUTPUT
print('\nRESULTS')
print(f'Maximum Exhaust Velocity at Vacuum       (ExpR -> Inf) = {v_max:.2f} m/s')
print(f'Maximum Exhaust Velocity at Design Point (ExpR = 2000) = {v_max_real:7.2f} m/s')